...
export default function BarChartWidget({ config, data }: BarChartWidgetProps) {
  ...
  const isStack    = Boolean(config.stack)
  ...
  // ── Horizontal single-series (ranking-style) ─────────────────────────────
  if (isHorizontal) {
    const n      = rows.length
    const yData  = rows.map((r) => String(xIdx >= 0 ? r[xIdx] ?? '' : r[0] ?? ''))
    const colIdx = yFields.length > 0 ? columns.indexOf(yFields[0]) : 1
    const values = rows.map((r) => toNum(colIdx >= 0 ? r[colIdx] : r[1]))
  ...
  const series = yFields.map((field, i) => {
    ...
    return {
      ...
      barMaxWidth: 36,
      itemStyle: {
        borderRadius: isStack ? undefined : [6, 6, 0, 0],
        color,
      },
